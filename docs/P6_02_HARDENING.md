...
| Backup alert | backup_alert_webhook_url | UID 1000, status read-only/state+egress만, DB/R2/key/profile/proxy/admin secret 없음 |
...
- `backupAlertWebhookUrl`: userinfo 없는 HTTPS backup failure receiver URL
...
프로필 복구는 백업에서 cookie를 복원하지 않고 재로그인한다. TLS 인증서·secret manager 자체의 재해복구는 별도 암호화/접근통제 정책으로 취급한다. P6-06 자동 백업·암호화·복구 훈련은 [P6-06 runbook](P6_06_DB_BACKUP.md)을, status-only backup failure 전달은 [P6-03 backup alert](P6_03_BACKUP_ALERTS.md)을 따른다. P6-02 자체 검증의 합성 archive는 제외 파일 누락만 증명한다.

...
| P6-02 Secret / Browser Profile 운영 Hardening | IMPLEMENTED_NOT_VALIDATED | DEC-20260915-005~007, Linux file secret·DB/proxy 교체·API/Worker/Caddy·metadata/log scan·UID negative·실제 profile 재사용/만료·backup exclusion·backup key/webhook provisioning 회귀 PASS | 실제 운영 host ACL/secret manager, R2·Provider/live alert receiver 검증 및 remote CI 확인 |
...
| P6-06 DB Backup 자동화 / 암호화 / 복구 검증 | IMPLEMENTED_NOT_VALIDATED | DEC-20260915-006/007, PostgreSQL 18.6 custom dump→AES-256-GCM→분리 volume→빈 DB restore/data 대조, wrong key/credential·26h stale 감지, 7/4/3 보존 및 P6-03 delivery/dedup/recovery Docker PASS | 실제 private R2 backup prefix·운영 host 장기 scheduler/live external alert receiver·remote CI 검증 필요 |
| P6-10 Production Docker / Reverse Proxy / HTTPS | IMPLEMENTED_NOT_VALIDATED | DEC-20260915-002/006, 공개 Quick Tunnel TLS 1.3·DNS·Caddy auth·내부망 격리·R2 Browser run 및 재시작 E2E PASS, P6_10_STAGING.md; backup 서비스 Compose 연결 | custom-domain Linux VM/Caddy ACME·실제 host firewall, P6-06 live R2/scheduler, remote CI 잔여; 일반 test runner 종료 문제 별도 |

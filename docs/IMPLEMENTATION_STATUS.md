...
| P6-02 Secret / Browser Profile 운영 Hardening | IMPLEMENTED_NOT_VALIDATED | DEC-20260915-005/006, Linux file secret·DB/proxy 교체·API/Worker/Caddy·metadata/log scan·UID negative·실제 profile 재사용/만료·backup exclusion·backup key provisioning 회귀 PASS | 실제 운영 host ACL/secret manager, R2·Provider live rotation 및 remote CI 확인 |
...
| P6-06 DB Backup 자동화 / 암호화 / 복구 검증 | IMPLEMENTED_NOT_VALIDATED | DEC-20260915-006, PostgreSQL 18.6 custom dump→AES-256-GCM→분리 volume→빈 DB restore/data 대조 0.984초, wrong key/credential·26h stale 감지, 7/4/3 보존 unit PASS | 실제 private R2 backup prefix·운영 host 장기 scheduler/P6-03 알림·remote CI 검증 필요 |

...
| P5-06 Demo Flow/Test Harness + P5-09/10 evidence | IMPLEMENTED_NOT_VALIDATED | DEC-20260914-014, 실제 Chromium demo lifecycle이 start/final-or-failure PNG·trace.zip·redacted result.json을 local ObjectStorage에 기록하는 경로 PASS | disposable PostgreSQL/pg-boss 실행과 원격 CI PASS 후 상태 확정 |
| P6-01 인증 경계 + P5 durable DB/Queue 연결 | IMPLEMENTED_NOT_VALIDATED | DEC-20260914-014, Caddy Basic Auth actor overwrite·loopback proxy token·same-origin JSON gate·artifact signed preview API·browser.run Worker/DB 구현; lint/typecheck/build 및 관련 21개 test PASS | `TEST_DATABASE_URL`로 browser durable DB/pg-boss integration와 실제 Caddy config validate를 실행; 원격 CI 미실행 |

...
export function maskSensitiveText(value: string): string {
  return value
    ...
    .replace(
      /([?&](?:access_token|api_key|key|signature|token|x-amz-signature)=)[^&#\s]*/gi,
      `$1${secretRedactionCensor}`,
    )
    .replace(
      /\b(password|secret|signature|x-amz-signature|token|access[_-]?token|api[_-]?key|cookie)\s*[:=]\s*[^\s,;]+/gi,
      `$1=${secretRedactionCensor}`,
    )
    ...
    hooks: {
      logMethod(inputArguments, method) {
        const safeArguments = inputArguments.map((argument) =>
          typeof argument === "string" ? maskSensitiveText(argument) : argument,
